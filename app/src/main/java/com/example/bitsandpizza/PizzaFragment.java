...
public class PizzaFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        int len = Pizza.pizzas.length;
        String[] pizzasName = new String[len];
        int[] pizzasImage = new int[len];
        for (int i = 0; i < pizzasName.length; i++) {
            pizzasName[i] = Pizza.pizzas[i].getName();
            pizzasImage[i] = Pizza.pizzas[i].getImageResourceId();
        }

        CaptionedImagesAdapter adapter = new CaptionedImagesAdapter(pizzasName, pizzasImage);
